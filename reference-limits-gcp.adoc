...
| *Storage Virtual Machines (SVMs)* | Die maximale Anzahl für Cloud Volumes ONTAP | Eine Datenservice-SVM und eine Ziel-SVM für die Disaster Recovery verwendet. Sie können die Ziel-SVM für den Datenzugriff aktivieren, wenn ein Ausfall auf der Quell-SVM auftritt. ^1^

Die eine Datenservice-SVM erstreckt sich über das gesamte Cloud Volumes ONTAP System. 
